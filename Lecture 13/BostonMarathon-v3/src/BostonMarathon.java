...
class ExamplesMarathon {
  ...
  ILoR expectedList2PositionUnder50 = 
      new ConsLoR(johnny, new ConsLoR(joan, mtlist));
  
  ILoR expectedList2FinishUnder4Hours =
      new ConsLoR(frank, new ConsLoR(bill, new ConsLoR(joan, mtlist)));
  
  ILoR expectedList2YoungerThan40 =
      new ConsLoR(frank, new ConsLoR(bill, new ConsLoR(joan, mtlist)));
  ...
  boolean testFindMethods(Tester t) {
    ...
    return
        // Male runners 
        t.checkExpect(list1.find(new RunnerIsMale()),
        new ConsLoR(johnny, mtlist))
        // Female runners
        && t.checkExpect(list1.find(new RunnerIsFemale()),
            new ConsLoR(joan, mtlist))
        // Position under 50
        && t.checkExpect(list2.find(new RunnerIsInFirst50()),
            expectedList2PositionUnder50)
        // Finish in under 4 hours
        && t.checkExpect(list2.find(new RunnerFinishUnder4Hours()),
            expectedList2FinishUnder4Hours)
        // Runners younger than 40
        && t.checkExpect(list2.find(new RunnerYoungerThan40()),
            expectedList2YoungerThan40);
  }
